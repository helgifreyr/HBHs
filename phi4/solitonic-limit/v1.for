...
c      WRITE(6,1900) TIME
     ...
     &         U1,U2,IND,IER,U01E1,U02E1,U03E1,U04E1,FDS500)
        ...
        WRITE(1,789)  U1(1,1,1)
        WRITE(1,789)  U1(1,1,2) 
        WRITE(1,789)  U1(1,1,3) 
        WRITE(1,789)  0. 
        WRITE(1,789)  0. 
...
c                       write(6,*)"TH*IS HERE"
	...
	ff=1.d0-X(I)
     ...
     -          U5y**2)))/dExp(2.*U3) + 
     -  2.*(-1.*dExp(2.*U1 - 2.*U2)*nr**2*U4**2 + 
     -     sn**2*(ff**4*r**2*U4x**2 + U4y**2) + 
     ...
     -       + 8.*dExp(2.*(U1 + U3))*nr**2*
     -       U4**2 + 4.*dExp(2.*(U1 + U2 + U3))*
     -       r**2*sn**2*U4**2*(c3 + c2*U4**2) + 
     -      dExp(4.*U2)*sn**4*
     -       (U5**2 - 2.*ff**2*r*U5*U5x + ff**4*r**2*U5x**2 + 
     -         U5y**2)))/dExp(2.*(U2 + U3))  
     ...
     -      4.*dExp(2.*(U1 + U3))*r**2*sn*U4**2*
     -       (c3 + c2*U4**2) - 
     -      1.*dExp(2.*U2)*sn**3*
     -       (U5**2 - 2.*ff**2*r*U5*U5x + ff**4*r**2*U5x**2 + 
     -         U5y**2) - 8.*dExp(2.*U1)*sn*U4**2*
     -       (nr*U5 - 1.*r*w)**2))/dExp(2.*U3)  

      eq4=  -1.*dExp(2.*U1 - 2.*U2)*nr**2*U4 + 
     -  cs*sn*U4y + sn**2*(-1.*dExp(2.*U1)*r**2*
     -      U4*(c3 + 2.*c2*U4**2) + 2.*ff**2*r*U4x + 
     -     ff**4*r**2*U2x*U4x + ff**4*r**2*U3x*U4x + 
     -     ff**3*r**2*(-2.*U4x + ff*U4xx) + U2y*U4y + U3y*U4y + 
     -     U4yy + dExp(2.*U1 - 2.*U3)*U4*
     -      (nr*U5 - 1.*r*w)**2)
...
c    	write(2,*)r,eq5 
	...
	Ttot=  2.*U4**2*(c3 + c2*U4**2 - 
     -    (2.*w*(-1.*nr*U5 + r*w))/(dExp(2.*U3)*r)
     -    )  

      T44= -1.*c2*U4**4 - (1.*(ff**4*r**2*U4x**2 + U4y**2))/
     -   (dExp(2.*U1)*r**2) + 
     -  U4**2*(-1.*c3 - (1.*
...
cccccccccccccccccccccccccccccccccccccccccccccc
     ...
     & (-0.5*sn**2*(dExp(2.*U2)*sn**2*
     &        (U5 - 1.*ff**2*r*U5x) - 
     &       8.*dExp(2.*U1)*nr*U4**2*(nr*U5 - 1.*r*w))
     &)/dExp(2.*U3)
     &
       Vxx21=0.
       Vxy21=0.
     ...
     &     8.*dExp(2.*U1)*nr*sn*U4**2*(nr*U5 - 1.*r*w)
     &)/dExp(2.*U3)
     &
       Vxx41=0.
       Vxy41=0.
     ...
     &
       ...
       Vy54=0.
       V54=
     & -16.*dExp(2.*U1 - 2.*U2)*nr*U4*(nr*U5 - 1.*r*w)
     &
       Vxx55=ff**4*r**2*sn**2
       Vxy55=0.
       Vyy55=sn**2
